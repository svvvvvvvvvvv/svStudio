...
def t_single_engine():
    ...
    check('★★ 端口上已经有一个引擎 ⇒ 第二个**自己退出**（非 0，且不挂住）',
          rc == 1, 'rc=%r\n%s' % (rc, out.strip()[:400]))
    # ⚠ 这里**不能**只查「已经有一个」这个子串 —— 那句 bind 兜底话术里也有它
    #   （"多半是已经有一个引擎在跑 —— 见上一段的三种处理办法"）。
    #   破法当场抓过一次：把 `if _port_taken(...)` 改成 `if False:`，字串照样在、检查照绿。
    #   ⇒ 换一把尺子：认**第一步探测**特有的那句「不会开第二个」，并且**排除** bind 兜底那句。
    check('★★ 而且**说人话**讲清为什么，且是**第一步探测**挡下的（不是撞到 bind 报错才退）',
          '不会开第二个' in out and '绑不上' not in out, out.strip()[:300],
          '探测那道闸没走通、只剩 bind 报错兜底 ⇒ 万一 allow_reuse_address 又被打开，就静默双绑了')
    check('★ 退的时候**带上端口号**（不看日志也能对上是谁占着）', str(busy) in out,
          out.strip()[:200])
