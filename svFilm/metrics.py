...
# 做卷时要"落到带里"的四条（见 master_doc_0912.md §五 · SKILL §44）
#
# ⚠⚠⚠ 这不是逐图 pass/fail 判据（P2-7, 09-13 评审）⚠⚠⚠
#   `BAND` 描述的是**组级**散布 —— 只该问"我这一卷的跨图统计中位，落在大师的范围内吗？"，
#   别给单张图打勾打叉（大师自己的单张也有一堆落在带外）。
#   ⚠ 而且它对**曝光很敏感**：入口零点一挪，`c_ratio` 就可能整条翻出去
#     （09-13 实测：入口「乙」之前 2.10~2.62、之后 3.47~3.68）。
#
# ★★ 尺子必须"比噪声宽"，否则必然假报（09-14 查清，SV 拍板「甲」；细节见 SKILL §44）
#   旧 `c_ratio` 口径 = 九位作者「中位」的 min~max ⇒ 上/下沿就是那 9 个数里最大/最小的，
#     「九人全在带内」**是恒真句、零信息**。实测：旧带宽只有 0.76，
#     而"大师随机抓 34 张"自身的波动就有 1.14 ⇒ **带子比噪声还窄 ⇒ 拿它卡批次必然假报超带**
#     （09-14 全库回归就因此误报"五卷彩度形状超带"，其实 neutral 只到 P89、portra400 才到 P97）。
#   ⇒ `c_ratio` 改成「**大师随机抽 34 张时、批次中位之比的 P5~P95**」：
#     它直接回答"我这一批跟大师能不能区分开"，随机抽样下天然只有约 10% 落在带外。
#     可复现出处：`_debug/lab_band_recal.py`（读 master_resurvey.json n=1172 / seed 20260914 / 20000 次）。
#   其余三条**仍用旧 min~max 口径**：它们本来就比噪声宽
#     （gray_pct 0.28>0.17 · c90 14.2>8.6 · black 13.0>5.0）⇒ 不会假报，暂不动。
#     将来若要统一改口径，跑同一个脚本即可。
BAND = {
    'gray_pct': (0.47, 0.75),
    'c_ratio': (2.46, 3.60),
