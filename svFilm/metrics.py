...
#   `BAND` = 「各作者线**中位**的 min~max」—— 它是**组级**的散布范围，不是"单张成片该落在哪"。
#   拿它给单张图打勾打叉会得出荒谬结论（大师自己的单张也有一堆落在带外）。
#   它只该用来问："**我这一卷的跨图统计中位**，落在九条作者线的中位区间里吗？"
#   真正该看的是趋势与象限，不是某一项越界没有。
#   ⚠ 而且它对**曝光很敏感**：入口零点一挪，`c_ratio`（彩度形状）就可能整条翻出带外
#     （09-13 实测：入口「乙」之前是 2.10~2.62 偏低，之后变成 3.47~3.68 偏高）。
BAND = {
    'gray_pct': (0.47, 0.75),
    'c_ratio': (2.65, 3.41),
    'c90': (11.7, 25.9),
