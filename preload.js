...
contextBridge.exposeInMainWorld('api', {
  ...
  resetColorGrade: (dirPath) => ipcRenderer.invoke('reset-color-grade', dirPath),
   ...
   *  ⚠ 进度走 `onExportBatchProgress` 事件，不是等返回值 —— 几百张要跑几十分钟。 */
  exportBatch: (payload) => ipcRenderer.invoke('export-batch', payload),
  exportBatchCancel: () => ipcRenderer.invoke('export-batch-cancel'),
  onExportBatchProgress: (cb) => ipcRenderer.on('export-batch-progress', (_e, o) => cb(o)),
  /* ---- 调色配方按目录存（每个目录一份，跟着目录走） ---- */
  getGrade: (dirName) => ipcRenderer.invoke('get-grade', dirName),
  setGrade: (dirName, grade) => ipcRenderer.invoke('set-grade', dirName, grade),
