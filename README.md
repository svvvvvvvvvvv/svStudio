...
**只有一条路**（09-15 定）：照片已经在硬盘上 ⇒ 左栏点**「加入目录」**，
把那个文件夹挂进图库列表 —— **原地读、一个字节都不复制**。
原目录被删 / 改名，那条就变成「找不到」（不静默消失）。

- **纯 RAW 的目录**（没有 JPG）：工作台会顺手给它生成一份**预览小图**，写进应用缓存
  （`%APPDATA%\svstudio\extpreview\`），**源目录只读**；出图仍然用源目录里的 RAW。
- 左栏底部那一行是**当前照片库根目录**（点「换图库」可换）。它只决定"库里有哪些主题"，
  不影响「加入目录」进来的东西。

> 早先还有一套「导入照片」（插卡 / U 盘 → 复制进库 → 按「日期_主题_地点」建档），
> **09-15 已整条删掉**：多一条复制路径就多一份"两边规则慢慢长歪"的风险，
> 而实际用法本来就是"RAW 在哪就在哪看"。
