...
| L1 | `tone.py` | lin → lin | 影调修正 | **靶是绝对靶**；曲线只作用在亮度上；可解析求中灰；**过亮护栏阈值 = 大师逐图「中位 L*」的 P95（`GUARD_MID`，灰阶 218），不是中灰 140** |
...
- **L3 肤色正向达标**：**逐像素**只向上补到大师脸区档（a* 16.3 / b* 18.5）+ 两轴都低才补 + 逐像素过亮门。单测 **128 项**。
